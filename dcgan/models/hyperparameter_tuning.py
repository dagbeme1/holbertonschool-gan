...
def discriminator_loss(real_output, fake_output):
    ...
    Args:
    real_output (tf.Tensor): Real images output from the discriminator.
    fake_output (tf.Tensor): Fake images output from the discriminator.
...
def generate_and_save_images(model, epoch, test_input):
    """
    Generate images using the generator model and save them.

    Args:
    model (tf.keras.Model): The generator model.
    epoch (int): The current training epoch.
    test_input (tf.Tensor): Random input noise for generating images.
